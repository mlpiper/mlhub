...
def create_module_graph(module_spec):
  ...
  with tf.Graph().as_default() as graph:
    resized_input_tensor = tf.placeholder(tf.float32, [None, height, width, 3], name="input_image")
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        mlops.set_stat("steps", i)
        mlops.set_stat("train accuracy", train_accuracy * 100)

        # multiply by 1 to convert into double until mlops supports float32
        mlops.set_stat("cross entropy", cross_entropy_value * 1)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--tfhub_module',
      type=str,
      # network doesn't always work. Use downloaded version by default
      # default=(
          # 'https://tfhub.dev/google/imagenet/inception_v3/feature_vector/1'),
      default=(
          'file:///data-lake/tensorflow/Inception/gitHubDownload'),
      help="""\
