...
class PmmlRESTfulServing(RESTfulComponent):
    JAVA_COMPONENT_ENTRY_POINT_CLASS = "com.parallelm.mlcomp.ComponentEntryPoint"
    JAVA_COMPONENT_CLASS_NAME = "org.mlpiper.mlhub.components.restful.PmmlModelServing"

    class Java:
        implements = ["org.mlpiper.mlops.MLOps"]

    def __init__(self, engine):
        super(PmmlRESTfulServing, self).__init__(engine)
    ...
    def _launch_custom_java_gateway(self, local_mode=False):
        self._run_java_server_entry_point(local_mode)
        self._setup_py4j_client_connection()

    def _run_java_server_entry_point(self, local_mode=False):
        ...
        self._java_port = PmmlRESTfulServing.find_free_port()
        cmd = ["java", "-cp", java_cp, PmmlRESTfulServing.JAVA_COMPONENT_ENTRY_POINT_CLASS, "--class-name",
               PmmlRESTfulServing.JAVA_COMPONENT_CLASS_NAME, "--port", str(self._java_port)]
        if self._verbose:
    ...
    def load_model_callback(self, model_path, stream, version):
        ...
        if self._component_via_py4j is None:
            self.set_wid(0)
            self._launch_custom_java_gateway(local_mode=True)
    ...
    def predict(self, query_string, body_data):
        ...
        if self._model_loaded:
            ...
            return 404, '{"error": "Pmml model was not loaded yet!"}'
...
if __name__ == '__main__':
    ...
    PmmlRESTfulServing.run(port=args.port, model_path=args.input_model)
