...
class DatasetDownloader(ConnectableComponent):
    ...
    def __init__(self, engine):
        ...
        DatasetDownloader.logger = self._logger
    ...
    def _init_params(self):
        ...
        if not self._dataset_url:
            self._dataset_url = DatasetDownloader.DEFAULT_DATASET_URL
    ...
    def _download_report_hook(chunk_number, chunk_max_size, download_total_size):
        now = time.time()
        if now - DatasetDownloader.last_update_time > 1.0:
            DatasetDownloader.last_update_time = now
            percent = 100.0 * chunk_number * chunk_max_size / download_total_size
            DatasetDownloader._download_report(percent)

    @staticmethod
    def _download_report(percent):
        DatasetDownloader.logger.info("Dataset download in progress ... {:.2f}%".format(percent))
